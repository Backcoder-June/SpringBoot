...
// 회원가입 business logic 만들기
    // join
    private final memberrepository A = new implementrepository();       //clientmember(domain) 가져올때 자동 import 안되면
                                                                    // 직접 import 해줌 (clientid, clientName 가진 domain)
        //Long 은 domain 에서 client id datatype 이였고, 여기서 임의로 id 뽑을거라 Long 준듯,
    public Long join(clientmemeber member){     //domain type 의 member data(id,Name) 를 주기,

        //중복이름 가입불가 business logic
